export const craftingProjects = {
    scrapCompressor: {
        id: 0,
        name: `Scrap Compressor`,
        totalCrafted: 0,
    },
    scavengersToolkit: {
        id: 1,
        name: `Scavenger's Toolkit`,
        totalCrafted: 0,
    },
    scrapAggregator: {
        id: 2,
        name: `Scrap Aggregator`,
        totalCrafted: 0,
    },
    resourceRefinery: {
        id: 3,
        name: `Resource Refinery`,
        totalCrafted: 0,
    },
    recyclingPlant: {
        id: 4,
        name: `Recycling Plant`,
        totalCrafted: 0,
    },
    materialFabricator: {
        id: 5,
        name: `Material Fabricator`,
        totalCrafted: 0,
    },
    materialSynthesizer: {
        id: 6,
        name: `Material Synthesizer`,
        totalCrafted: 0,
    },
    techSalvageMatrix: {
        id: 7,
        name: `Tech Salvage Matrix`,
        totalCrafted: 0,
    },
    innovationLab: {
        id: 8,
        name: `Innovation Lab`,
        totalCrafted: 0,
    },
    nexusCoreIntegration: {
        id: 9,
        name: `Nexus Core Integration`,
        totalCrafted: 0,
    },
    generatorOverclocker: {
        id: 10,
        name: `Generator Overclocker`,
        totalCrafted: 0,
    },
    masterScavengersBlueprint: {
        id: 11,
        name: `Master Scavenger's Blueprint`,
        totalCrafted: 0,
    },
}